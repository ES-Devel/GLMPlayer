...
def update_config(libdir, values = {}):
    ...
    filename = os.path.join(libdir, 'glmplayer_lib/glmplayerconfig.py')
    oldvalues = {}
    try:
        fin = file(filename, 'r')
...
def update_desktop_file(filename, target_pkgdata, target_scripts):
    ...
    try:
        ...
        for line in fin:
            if 'Icon=' in line:
                line = "Icon=%s\n" % (target_pkgdata + 'media/glmplayer.svg')
            elif 'Exec=' in line:
                cmd = line.split("=")[1].split(None, 1)
                line = "Exec=%s" % (target_scripts + 'glmplayer')
                if len(cmd) > 1:
...
class InstallAndUpdateDataDirectory(DistUtilsExtra.auto.install_auto):
    def run(self):
        ...
        values = {'__template_data_directory__': "'%s'" % (target_pkgdata),
