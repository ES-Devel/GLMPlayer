...
##################################################################################	
	def __init__(self):
		...
		proc.wait()
		self.time_song = 0
		self.imagen.set_from_file("interface/NOCD.png")
		############################################################################
		############################################################################
...
# MANEJADORES DE REPRODUCCION Y MAPEO DE LISTA
	def REP(self):
		...
		try:
			...
			with open('interface/artwork.png', 'wb') as img:
				img.write(artwork)
			self.imagen.set_from_pixbuf(gtk.gdk.pixbuf_new_from_file_at_size('interface/artwork.png', 200, 200))
		except:
			self.imagen.set_from_file("interface/NOCD.png")
